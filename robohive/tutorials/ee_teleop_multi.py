...
def poll_keyboard(input_device):
    ...
    if sen == 'f': # move right
        delta_pos[0] = 1
    elif sen == 's': # Move left
        delta_pos[0] = -1
    elif sen == 'e': # Move forward
        delta_pos[1] = 1
    elif sen == 'd': # Move backward
        delta_pos[1] = -1
    elif sen == 'q': # Raise
        delta_pos[2] = 1
    elif sen == 'z': # Lower
...
def poll_gamepad(input_device):
    ...
    # positions
    delta_pos = np.array([0, 0, 0])
    # Moving EE left or right, assumes cam. facing robot
    delta_pos[0] = - sen["ABS_X"]
    # Moving EE forward or backward, assumes cam. facing robot
    delta_pos[1] = sen["ABS_Y"]
    # Raise or lower
    delta_pos[2] = sen["ABS_Z"] - sen["ABS_RZ"]

    # rotations
    delta_euler = np.array([0, 0, 0])
    # TODO: switch up down and left right on the keyboard polling itself.
    if sen["ABS_HAT0X"] == -1:
        delta_euler[0] = -1
    elif sen["ABS_HAT0X"] == 1:
        delta_euler[0] = 1
    elif sen["ABS_HAT0Y"] == 1: # forward rot
        delta_euler[1] = -1
    elif sen["ABS_HAT0Y"] == -1: # backward rot
        delta_euler[1] = 1
    elif sen["BTN_TL"] == 1: # X-axis left rot.
        delta_euler[2] = -1
